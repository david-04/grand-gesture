...
((global, factory) => {
    ...
    var URI = GBK => {
        ...
        function getModue(passChars) {
            ...
            var isPass = s => ~passChars.indexOf(s) || /[0-9a-zA-Z]/.test(s);
            return {
                encode: str =>
                    (str + "").replace(/./g, v => {
                ...
                decode: enstr =>
                    enstr.replace(/(%[\dA-Za-z]{2})+/g, (a, b, c) => {
                        var str = "";
                        var arr = a.match(/.../g);
                        for (var i = 0; i < arr.length; i++) {
                            var hex = arr[i];
                            var code = Number.parseInt(hex.substr(1), 16);
                            if (code & 0x80) {
                                str += GBK.decode([code, Number.parseInt(arr[++i].substr(1), 16)]);
                            } else {
    ...
    var Fn_Hex_decode = function decode() {
        ...
        for (var i = 0, w = str.length; i < w; i++) {
            ...
            if (code < 38 || code > 126) return Number.NaN;
    ...
    var Fn_unzip = function unZip() {
        return arguments[0]
            .replace(/\x23(\d+)\x24/g, (a, b) => Array(+b + 4).join("#"))
            .replace(/[\x26-\x7e]\x25[\x26-\x7e]/g, a => {
            ...
            .replace(/\x23/g, "###")
            .replace(/([\x26-\x7e]{2})\x21([\x26-\x7e\x23]+)(?:\x20|$)/g, (all, hd, dt) =>
                dt.replace(/./g, a => {
                    if (a != "#") {
                        return hd + a;
                    } else {
