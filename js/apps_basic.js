...
sue.apps = {
    ...
    initBox: boxInfo => {
        ...
        domBox = sue.apps.domCreate(
            "smartup",
            { setName: ["className"], setValue: ["su_apps"] },
            null,
            "z-index:" + Number.parseInt(new Date().getTime() / 1000),
            { setName: ["appname"], setValue: [boxInfo.appName] }
        );
        //head
        ...
        if (boxInfo.headTitle) {
            const _domHeadTtitle = sue.apps.domCreate(
                "span",
                { setName: ["className"], setValue: ["su_title"] },
                null,
                null,
                null,
                sue.apps.i18n(boxInfo.headTitle)
            );
            domHead.appendChild(_domHeadTtitle);
        }
        if (boxInfo.headCloseBtn) {
            const _domHeadCloseBtn = sue.apps.domCreate(
                "div",
                { setName: ["className"], setValue: ["su_btn_close"] },
                null,
                null,
                null,
                "x"
        ...
        if (boxInfo.menu) {
            ...
            for (var i = 0; i < boxInfo.menu.length; i++) {
                var _domMenuImg = sue.apps.domCreate(
                    "img",
                    { setName: ["className"], setValue: [boxInfo.menu[i].className] },
                    null,
                    null,
                    boxInfo.menu[i].action ? { setName: ["action"], setValue: [boxInfo.menu[i].action] } : null
                );
        ...
        if (boxInfo.options) {
            ...
            for (var i = 0; i < boxInfo.options.length; i++) {
                if (boxInfo.options[i].type == "select") {
                    var _label = sue.apps.domCreate(
                        "label",
                        { setName: ["className"], setValue: ["options_labelname"] },
                        null,
                        null,
                        null,
                        sue.apps.i18n(boxInfo.options[i].name)
                    );
                    ...
                    for (var ii = 0; ii < boxInfo.options[i].value.length; ii++) {
                        var _domOption = sue.apps.domCreate(
                            "option",
                            null,
                            null,
                            null,
                            null,
                            sue.apps.i18n(boxInfo.options[i].value[ii])
                        );
                ...
                if (boxInfo.options[i].type == "checkbox") {
                    var _time = Number.parseInt(new Date().getTime() / 1000);
                    ...
                    var _label = sue.apps.domCreate(
                        "label",
                        { setName: ["className"], setValue: ["options_labeldes"] },
                        null,
                        null,
                        null,
                        sue.apps.i18n(boxInfo.options[i].name)
                ...
                if (boxInfo.options[i].type == "range") {
                    var _label = sue.apps.domCreate(
                        ...
                        null,
                        null,
                        sue.apps.i18n(boxInfo.options[i].label)
                    );
            ...
            //button box
            const _btnBox = sue.apps.domCreate("div", {
                setName: ["className"],
                setValue: ["options_btnbox"],
            });
            const _btnCancel = sue.apps.domCreate("input", {
                setName: ["className"],
                setValue: ["options_btn_cancel"],
            });
            _btnCancel.type = "button";
            _btnCancel.value = sue.apps.i18n("btn_cancel");
            const _btnSave = sue.apps.domCreate("input", {
                setName: ["className"],
        ...
        switch (e.type) {
            case "click":
                ...
                if (e.target.classList.contains("su_head") || e.target.classList.contains("su_title")) {
                    ...
                    sue.apps.getAPPboxEle(e).style.cssText += "z-index:" + Number.parseInt(mytime / 1000);
                }
                break;
            case "mousedown":
                if (
                    e.button == 0 &&
                    (e.target.classList.contains("su_head") || e.target.classList.contains("su_title"))
                ) {
                    var boxposX = e.target.classList.contains("su_head")
                            ? e.target.parentNode.offsetLeft
                            : e.target.parentNode.parentNode.offsetLeft,
                        boxposY = e.target.classList.contains("su_head")
    ...
    saveConf: e => {
        ...
        chrome.runtime.sendMessage({ type: "apps_saveconf", apptype: _appname, config: _config }, response => {
            console.log(response);
            // response&&response.type?sue.apps.notification(e.target,response.value):null;
            !response || !response.type || !config.general.settings.appnotif
                ? null
                : sue.apps.notification(e.target, response.value);
        });
        sue.apps.showOpt(e);
    },
    getAPPboxEle: e => {
    ...
    boxMove: e => {
        ...
        OBJ.style.cssText +=
            "transition:none;" +
            "left:" +
            (e.clientX - sue.apps.cons.boxmove.posX) +
            "px;" +
            "top:" +
            (e.clientY - sue.apps.cons.boxmove.posY) +
            "px;" +
            "z-index:" +
            Number.parseInt(mytime / 1000);
    },
    boxClose: e => {
    ...
    initPos: e => {
        const dom = sue.apps.getAPPboxEle(e.target || e);
        const _fn = () => {
    ...
    editBoxInit: (e, opt) => {
        ...
        const boxBtn = sue.apps.domCreate("div", {
            ...
            boxBtnClose = sue.apps.domCreate(
                "button",
                { setName: ["className"], setValue: ["su_editbtncancel"] },
                null,
                null,
                null,
            ...
            boxBtnSave = sue.apps.domCreate(
                "button",
                { setName: ["className"], setValue: ["su_editbtnadd"] },
                null,
                null,
                null,
    ...
    editBoxClose: e => {
        ...
        const domEdit = dom.querySelector(".su_editbox"),
            domBg = dom.querySelector(".su_editbg");
        domEdit.remove();
        domBg.remove();
    },
    notification: (e, text, type, time) => {
        text = text || sue.apps.i18n("msg_saved");
        type = type || "success";
        time = time || "2000";
        const _dom = sue.apps.getAPPboxEle(e);
        const _domNotif = sue.apps.domCreate(
            "div",
            { setName: ["className"], setValue: ["su_notifi"] },
            null,
            null,
            null,
            text
    ...
    showPanel: dom => {
        console.log(dom);
        const _show = window.getComputedStyle(dom).opacity == 0 ? true : false;
        if (_show) {
