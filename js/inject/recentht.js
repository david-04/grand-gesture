...
sue.apps.recentht = {
    ...
    initUI: function () {
        ...
        for (var i = 0; i < this.ht.length; i++) {
            var _li = sue.apps.domCreate(
                "li",
                { setName: ["className"], setValue: ["su_recentht_li"] },
                null,
                null,
    ...
    handleEvent: e => {
        switch (e.type) {
            case "click":
                if (
                    e.target.classList.contains("su_recentht_li") ||
                    e.target.parentNode.classList.contains("su_recentht_li")
                ) {
...
chrome.runtime.sendMessage({ type: "apps_getvalue", apptype: "recentht" }, response => {
