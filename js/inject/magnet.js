...
sue.apps.magnet = {
    initUI: function () {
        ...
        const theAppBox = sue.apps.domCreate("div", {
            setName: ["className"],
            setValue: ["su_magnetbox"],
        });
        dom.querySelector(".su_main").appendChild(theAppBox);

        const domLeft = sue.apps.domCreate("div", {
            setName: ["className"],
            setValue: ["su_magenet_domleft"],
        });
        const domInput = sue.apps.domCreate("textarea", {
                setName: ["className", "placeholder"],
                setValue: ["su_magenet_input", sue.apps.i18n("magnet_placeholder_input")],
            }),
            btnClear = sue.apps.domCreate(
                "button",
                {
                    setName: ["className"],
                ...
                null,
                null,
                sue.apps.i18n("btn_clear")
            ),
            btnDone = sue.apps.domCreate(
                "button",
                { setName: ["className"], setValue: ["su_magenet_btn su_magnet_done"] },
                null,
                null,
                null,
                sue.apps.i18n("btn_done")
            ),
            domOutput = sue.apps.domCreate("textarea", {
                setName: ["className", "disabled"],
                setValue: ["su_magenet_output", true],
            }),
            domLink = sue.apps.domCreate("a", {
                setName: ["className"],
                setValue: ["su_magenet_link"],
            });

        const domQr = sue.apps.domCreate("div", {
            setName: ["className"],
            setValue: ["su_magenet_qr"],
        });
    ...
    action: e => {
        const dom = e.target || e;
        const domOutput = dom.nextSibling,
...
chrome.runtime.sendMessage({ type: "apps_getvalue", apptype: "magnet" }, response => {
