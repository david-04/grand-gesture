...
sue.apps.appslist = {
    ...
    initUI: function () {
        ...
        const theAppBox = sue.apps.domCreate("div", {
            setName: ["className"],
            setValue: ["su_appslist_box"],
        });
        dom.querySelector(".su_main").appendChild(theAppBox);

        const _ul = sue.apps.domCreate("ul", {
            setName: ["className"],
            setValue: ["su_appslist_ulenabled"],
        });
        const _conf = sue.apps.appslist.config.enabled;
        for (var i = 0; i < _conf.length; i++) {
            ...
            var _div = sue.apps.domCreate(
                ...
                {
                    setName: ["className"],
                    setValue: ["su_appslist_li su_appslist_item"],
                },
                null,
                null,
                { setName: ["id"], setValue: [_conf[i]] },
    ...
    editMode: e => {
        if (e.target.classList.contains("su_appslist_editmode")) {
            ...
            sue.apps.getAPPboxEle(e).querySelector(".su_title").textContent =
                sue.apps.i18n("appslist") + " (" + sue.apps.i18n("editmode") + ")";
            sue.apps.appslist.editModeInit(e);
        }
    },
    editModeInit: e => {
        const _ulenabled = sue.apps.getAPPboxEle(e).querySelector(".su_appslist_ulenabled");
        _ulenabled.classList.add("su_appslist_ulenabled_before");

        const _uldisabled = sue.apps.domCreate("ul", {
            setName: ["className"],
            setValue: ["su_appslist_uldisabled"],
        });
        const _apps = sue.apps.appslist.apps,
            _enabled = sue.apps.appslist.config.enabled;
        for (var i = 0; i < _apps.length; i++) {
            if (!_enabled.contains(_apps[i])) {
                var _li = sue.apps.domCreate(
                    "li",
                    { setName: ["className"], setValue: ["su_appslist_li"] },
                    null,
                    null,
                    { setName: ["id"], setValue: [_apps[i]] }
                );
                var _div = sue.apps.domCreate(
                    "div",
                    {
        ...
        sue.apps.appslist.cons.sortDomEnabled = Sortable.create(_ulenabled, {
            ...
            onEnd: e => {
                console.log(e);
                const _lis = e.target.querySelectorAll("li.su_appslist_li");
                const _arr = [];
    ...
    saveConf: () => {
        chrome.runtime.sendMessage({
            type: "apps_saveconf",
            apptype: "appslist",
            config: sue.apps.appslist.config,
        });
    },
};
chrome.runtime.sendMessage({ type: "apps_getvalue", apptype: "appslist" }, response => {
