...
sue.apps.rss = {
    cons: {},
    initUI: () => {
        sue.apps.rss.indexID = Number.parseInt(new Date().getTime() / 1000);
        const appInfo = {
            appName: "rss",
            headTitle: "rss",
            headCloseBtn: true,
        ...
        //rss head
        const _rssHead = sue.apps.domCreate("div", {
            setName: ["className"],
            setValue: ["rss_head"],
        });
        const _rssTitle = sue.apps.domCreate("span", {
            setName: ["className"],
        ...
        //rss box
        const _rssBox = sue.apps.domCreate("div", {
            setName: ["className"],
            setValue: ["rss_box"],
        });
        const _rssSpan = sue.apps.domCreate("div", null, null, "color: #3698F9;");
        const _rssImg = sue.apps.domCreate("img", null, null, "display: inline-block;margin-bottom: -10px;");
        ...
        //sub box
        const _rssSub = sue.apps.domCreate("div", {
            setName: ["className"],
            setValue: ["rss_sub"],
        });
        const _subBox = sue.apps.domCreate("div", {
            setName: ["className"],
            setValue: ["sub_box"],
        });
        const _subText = sue.apps.domCreate("input", {
            setName: ["className"],
            setValue: ["sub_text"],
        });
        _subText.type = "text";
        _subText.placeholder = sue.apps.i18n("app_rss_subplace");
        const _subBtn = sue.apps.domCreate("input", {
            setName: ["className"],
            setValue: ["sub_add"],
        });
        _subBtn.type = "button";
        ...
        const _subItem = sue.apps.domCreate("ul", {
            setName: ["className"],
            setValue: ["sub_items"],
        });
    ...
    saveConf: () => {
        ...
        chrome.runtime.sendMessage(
            { type: "apps_saveconf", apptype: "rss", config: sue.apps.rss.config },
            response => {}
        );
    },
    fixURL: url => {
    ...
    rssAdd: e => {
        ...
        var domli = sue.apps.domCreate(
            "li",
            { setName: ["className", "title"], setValue: ["sub_item", theurl] },
            null,
            null,
            {
                setName: ["url", "id"],
        ...
        var domdel = sue.apps.domCreate(
            "span",
            { setName: ["className"], setValue: ["sub_itemdel"] },
            null,
            null,
            null,
            "x"
    ...
    menu: dom => {
        ...
        chrome.storage.local.get(items => {
            ...
            for (var i = 0; i < feed.length; i++) {
                var domli = sue.apps.domCreate(
                    "li",
                    { setName: ["className", "title"], setValue: ["sub_item", feed[i]] },
                    null,
                    null,
                    { setName: ["url", "id"], setValue: [feed[i], i] },
    ...
    rss: (dom, url) => {
        chrome.runtime.sendMessage({ type: "appsAction", app: "rss", action: "getMessage", value: url }, s => {
            console.log(s);
        });
    },
    itemOpen: e => {
    ...
    itemList: (message, sender, sendResponse) => {
        console.log(message);
        ...
        //rss head
        const rssheaddom = sue.apps.rss.dom.querySelector(".su_main .rss_head");
        rssheaddom.textContent = "";

        const _titleImg = sue.apps.domCreate("img");
        _titleImg.src = data.img;
        const _titleLink = sue.apps.domCreate("a");
        ...
        const rssdom = sue.apps.rss.dom.querySelector(".rss_box");
        rssdom.textContent = "";
        const _ul = sue.apps.domCreate("ul");
        for (var i = 0; i < data.items.length; i++) {
            var _li = sue.apps.domCreate(
                "li",
                { setName: ["className"], setValue: ["rss_item"] },
                null,
                null,
                { setName: ["link"], setValue: [data.items[i].link] },
                data.items[i].title
            );
            _ul.appendChild(_li);
        }
...
chrome.runtime.sendMessage({ type: "apps_getvalue", apptype: "rss" }, response => {
