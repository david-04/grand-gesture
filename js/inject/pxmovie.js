...
sue.apps.pxmovie = {
    ...
    initUI: function () {
        ...
        const theAppBox = sue.apps.domCreate("div", {
            setName: ["className"],
            setValue: ["su_pxmovie_box"],
        });
        dom.querySelector(".su_main").appendChild(theAppBox);

        theAppBox.appendChild(
            sue.apps.domCreate("div", { setName: ["className"], setValue: ["list"] }, null, "display:none;")
        );
        theAppBox.appendChild(
            sue.apps.domCreate("div", { setName: ["className"], setValue: ["data"] }, null, "display:none;")
        );
        theAppBox.appendChild(
            sue.apps.domCreate("img", {
    ...
    list: message => {
        ...
        const _boxList = dom.querySelector(".list"),
        ...
        for (var i = 0; i < value.length; i++) {
            var _li = sue.apps.domCreate(
                "li",
                { setName: ["className"], setValue: ["item"] },
                null,
                null,
                { setName: ["url"], setValue: [value[i][3]] },
                "[" + value[i][0] + "]" + value[i][2]
            );
            _ul.appendChild(_li);
        }
    ...
    data: message => {
        ...
        let value = message.value,
            dom = sue.apps.pxmovie.dom;
        dom = dom.querySelector(".su_pxmovie_box");
        const _data = dom.querySelector(".data"),
            _loading = dom.querySelector(".loading"),
            _name = sue.domCreate("div", { setName: ["className"], setValue: ["name"] }, null, null, null, value.name),
            _back = sue.apps.domCreate(
                "button",
                { setName: ["className"], setValue: ["back"] },
                null,
                "display: inline-block;",
                null,
                sue.apps.i18n("btn_back")
            );
        _info = sue.apps.domCreate("div", {
            setName: ["className"],
            setValue: ["info"],
        });
        _data.textContent = "";
        ...
        for (var i = 0; i < value.info.length; i++) {
            ...
            _infolist.appendChild(
                sue.apps.domCreate(
                    "span",
                    { setName: ["className"], setValue: ["infolist"] },
                    null,
                    null,
                    null,
                    value.info[i][0]
        ...
        for (var i = 0; i < value.dl.length; i++) {
            var _dl = sue.apps.domCreate("textarea", {
                setName: ["className", "value"],
                setValue: ["url", value.dl[i][1]],
            });
            _info.appendChild(_dl);
        }
        _info.appendChild(
            sue.apps.domCreate("div", { setName: ["className"], setValue: ["des"] }, null, null, null, value.des)
        );
