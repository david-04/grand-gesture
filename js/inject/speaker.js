...
sue.apps.speaker = {
    ...
    initUI: function () {
        ...
        const theAppBox = sue.apps.domCreate("div", {
            setName: ["className"],
            setValue: ["su_speakerbox"],
        });
        dom.querySelector(".su_main").appendChild(theAppBox);

        const _inputbox = sue.apps.domCreate("div", {
                setName: ["className"],
                setValue: ["speaker_inputbox"],
            }),
            _textarea = sue.apps.domCreate("textarea", {
                setName: ["className"],
                setValue: ["speaker_inputarea"],
            }),
            _br = sue.apps.domCreate("br"),
            _btnPlay = sue.apps.domCreate(
                "button",
                { setName: ["className"], setValue: ["speaker_btn_play"] },
                null,
                null,
                null,
                sue.apps.i18n("btn_play")
            ),
            _btnPause = sue.apps.domCreate(
                "button",
                { setName: ["className"], setValue: ["speaker_btn_pause"] },
                null,
                "display:none;",
                null,
                sue.apps.i18n("btn_pause")
            ),
            _btnStop = sue.apps.domCreate(
                "button",
                { setName: ["className"], setValue: ["speaker_btn_stop"] },
                null,
                null,
                null,
    ...
    speaker: e => {
        chrome.runtime.sendMessage({
            ...
            action: "speak",
            value: {
                type: e.target.className.substr(12),
                txt: sue.apps.getAPPboxEle(e).querySelector("textarea").value,
            },
        });
    },
};
chrome.runtime.sendMessage({ type: "apps_getvalue", apptype: "speaker" }, response => {
