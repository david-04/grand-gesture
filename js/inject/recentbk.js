...
sue.apps.recentbk = {
    ...
    initUI: function () {
        const appInfo = {
            ...
            options: [
                ...
                {
                    type: "range",
                    label: "app_recentbk_recentlength",
                    name: "n_num",
                    min: 5,
                    max: 50,
                },
        ...
        for (var i = 0; i < this.bk.length; i++) {
            var _li = sue.apps.domCreate(
                "li",
                { setName: ["className"], setValue: ["su_recentbk_li"] },
                null,
                null,
    ...
    handleEvent: e => {
        switch (e.type) {
            case "click":
                if (
                    e.target.classList.contains("su_recentbk_li") ||
                    e.target.parentNode.classList.contains("su_recentbk_li")
                ) {
                    ...
                    chrome.runtime.sendMessage({
                        ...
                        app: "recentbk",
                        action: "openItem",
                        value: sue.apps.recentbk.bk[theDom.dataset.id].url,
                    });
...
chrome.runtime.sendMessage({ type: "apps_getvalue", apptype: "recentbk" }, response => {
