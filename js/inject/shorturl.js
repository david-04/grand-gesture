sue.apps.shorturl = {
    ...
    initUI: function () {
        ...
        const theAppBox = sue.apps.domCreate("div", {
            setName: ["className"],
            setValue: ["su_shorturl_box"],
        });
        dom.querySelector(".su_main").appendChild(theAppBox);

        const domInfo = sue.apps.domCreate(
            "div",
            { setName: ["className"], setValue: ["su_shorturl_info"] },
            null,
            null,
            null,
            sue.apps.i18n("app_shorturl_wait")
        );
        const domAction = sue.apps.domCreate("div", {
                setName: ["className"],
                setValue: ["su_shorturl_action"],
            }),
            domQr = sue.apps.domCreate("div", {
                setName: ["className"],
                setValue: ["su_shorturl_qr"],
            });
        theAppBox.appendChild(domAction);
        theAppBox.appendChild(domQr);

        const domSourceText = sue.apps.domCreate(
                "textarea",
                {
            ...
            domKey = sue.apps.domCreate(
                "input",
                {
                    ...
                    setValue: [
                        "su_shorturl_textkey",
                        "text",
                        sue.apps.i18n("app_shorturl_keyholder"),
                        sue.apps.i18n("app_shorturl_keyholder"),
                    ],
                },
                null,
            ...
            domBtn = sue.apps.domCreate(
                "button",
                { setName: ["className"], setValue: ["su_shorturl_btn"] },
                null,
                null,
                null,
                sue.apps.i18n("app_shorturl_btn")
            ),
            domShortText = sue.apps.domCreate(
                "input",
                {
        ...
        const labelSource = sue.apps.domCreate(
                "label",
                { setName: ["className"], setValue: ["su_shorturl_labelsource"] },
                null,
                null,
                null,
                sue.apps.i18n("app_shorturl_source")
            ),
            labelkey = sue.apps.domCreate(
                "label",
                { setName: ["className"], setValue: ["su_shorturl_labelkey"] },
                null,
                "clear:both;",
                null,
                sue.apps.i18n("app_shorturl_key")
            ),
            labelshort = sue.apps.domCreate(
                "label",
                { setName: ["className"], setValue: ["su_shorturl_labelshort"] },
                null,
                "clear:both;",
                null,
                sue.apps.i18n("app_shorturl_short")
            );
    ...
    initValue: responseData => {
        console.log(responseData);
        if (responseData.status == "fail" && !responseData.shorturl) {
            sue.apps.shorturl.error(responseData.code);
            return;
        }
        const dom = sue.apps.shorturl.dom;
        const textSourceurl = dom.querySelector(".su_shorturl_textsource"),
    ...
    initQr: str => {
        const domQr = sue.apps.shorturl.dom.querySelector(".su_shorturl_qr");
        domQr.textContent = "";
        new QRCode(domQr, str);
    },
    getURL: e => {
        sue.apps.getAPPboxEle(e).querySelector(".su_shorturl_info").innerText =
            sue.apps.i18n("app_shorturl_Waitserver");
        chrome.runtime.sendMessage({
            type: "appsAction",
            app: "shorturl",
            action: "getURL",
            value: {
                key: sue.apps.getAPPboxEle(e).querySelector(".su_shorturl_textkey").value,
            },
        });
    },
    error: err => {
...
chrome.runtime.sendMessage({ type: "apps_getvalue", apptype: "shorturl" }, response => {
