...
sue.apps.synced = {
    ...
    initUI: function () {
        ...
        const _head = sue.apps.domCreate("div", {
                setName: ["className"],
                setValue: ["su_synced_head"],
            }),
            _spanSwitch = sue.apps.domCreate(
                "span",
                { setName: ["className"], setValue: ["su_synced_switch"] },
                null,
                null,
                null,
                sue.apps.i18n("app_synced_switch")
            ),
            _selectDevice = sue.apps.domCreate("select", {
                setName: ["className"],
                setValue: ["su_synced_device"],
            }),
            _box = sue.apps.domCreate("div", {
                setName: ["className"],
                setValue: ["su_synced_box"],
            });
        ...
        for (var i = 0; i < sue.apps.synced.sync.length && sue.apps.synced.sync.length > 0; i++) {
            _selectDevice.appendChild(
                sue.apps.domCreate("option", null, null, null, null, sue.apps.synced.sync[i].deviceName)
            );
        }
        ...
        for (var i = 0; sue.apps.synced.sync.length > 0 && i < sue.apps.synced.sync[0].sessions.length; i++) {
            ...
            for (var ii = 0; ii < theobj.length; ii++) {
                var _li = sue.apps.domCreate(
                    "li",
                    { setName: ["className"], setValue: ["su_synced_li"] },
                    null,
                    null,
                    { setName: ["id"], setValue: [theobj[ii].sessionId] },
                    theobj[ii].title
                );
                _ul.appendChild(_li);
...
chrome.runtime.sendMessage({ type: "apps_getvalue", apptype: "synced" }, response => {
