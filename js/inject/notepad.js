...
sue.apps.notepad = {
    ...
    initUI: function () {
        const appInfo = {
            ...
            options: [
                {
                    type: "checkbox",
                    label: "n_notepad_delconfirm",
                    name: "n_notepad_delconfirm",
                    checked: true,
                },
                {
                    type: "checkbox",
                    label: "n_notepad_switchsave",
                    name: "n_notepad_switchsave",
                    checked: true,
                },
                {
                    type: "checkbox",
                    label: "n_notepad_last",
                    name: "n_notepad_last",
                    checked: true,
                },
            ],
        ...
        const theAppBox = sue.apps.domCreate("div", {
            setName: ["className"],
            setValue: ["su_notepad_box"],
        });
        dom.querySelector(".su_main").appendChild(theAppBox);

        const _boxList = sue.apps.domCreate("div", {
                setName: ["className"],
                setValue: ["su_notepad_list"],
            }),
            _boxMain = sue.apps.domCreate("div", {
                setName: ["className"],
                setValue: ["su_notepad_main"],
            }),
            _boxBottom = sue.apps.domCreate("div", {
                setName: ["className"],
                setValue: ["su_notepad_bottom"],
            });

        _boxList.appendChild(sue.apps.domCreate("ul"));

        _boxMain.appendChild(
            sue.apps.domCreate("textarea", {
                setName: ["className"],
                setValue: ["su_notepad_content"],
            })
        );

        _boxBottom.appendChild(
            sue.apps.domCreate(
                "button",
                { setName: ["className"], setValue: ["su_notepad_btnadd"] },
                null,
                null,
                { setName: ["action"], setValue: ["add"] },
        ...
        _boxBottom.appendChild(
            sue.apps.domCreate(
                ...
                null,
                null,
                { setName: ["action", "id"], setValue: ["notepad-contentsave", 0] },
                sue.apps.i18n("btn_save")
            )
        );
        _boxBottom.appendChild(
            sue.apps.domCreate(
                "div",
                { setName: ["className"], setValue: ["su_notepad_des"] },
                null,
                null,
                null,
                sue.apps.i18n("des_notepad_bottom")
            )
        );
        ...
        const boxLock = sue.apps.domCreate("div", {
            ...
            lockEnable = sue.apps.domCreate("div", {
                setName: ["className"],
                setValue: ["su_notepad_lockenable"],
            }),
            lockDisable = sue.apps.domCreate("div", {
                setName: ["className"],
                setValue: ["su_notepad_lockdisable"],
            });
    ...
    itemInit: e => {
        ...
        const _id = Number.parseInt(ele.dataset.id);
        const data = sue.apps.notepad.cons.message.data.item;
    ...
    itemSwitch: (e, autosave) => {
        const _autoSave = autosave === undefined ? sue.apps.notepad.config.n_notepad_switchsave : autosave;
        if (_autoSave) {
            sue.apps.notepad.contentSave(sue.apps.getAPPboxEle(e).querySelector(".su_notepad_btnsave"));
        }
        const ele = e.target || e;
        const _lis = sue.apps.getAPPboxEle(ele).querySelectorAll("li");
    ...
    itemEdit: function (e) {
        const ele = e.target || e;
        sue.apps.notepad.itemSwitch(e);
        const domText = ele.parentNode.querySelector(".su_notepad_litext"),
    ...
    itemEditSave: e => {
        const _id = Number.parseInt(e.target.dataset.id);
        sue.apps.notepad.cons.message.data.item[_id].title =
            e.target.parentNode.querySelector(".su_notepad_litext").value;
        // chrome.runtime.sendMessage({type:"appsAction",app:"notepad",action:"DBAction",value:{
        // 	method:"put",
        // 	data:{
        // 		id:0,
    ...
    itemDel: e => {
        ...
        sue.apps.notepad.saveConf(e);
        sue.apps.notepad.listInit();
        sue.apps.notepad.itemSwitch(
            sue.apps.notepad.dom.querySelectorAll(".su_notepad_list li>div")[_id > 0 ? _id - 1 : 0],
            false
        );
    },
    listInit: () => {
        const data = sue.apps.notepad.cons.message.data.item;
        const _ul = sue.apps.notepad.dom.querySelector("ul");
        _ul.innerText = "";
        console.log(data);
        for (var i = 0; i < data.length; i++) {
            var _li = sue.apps.domCreate("li", { setName: ["className"], setValue: ["su_notepad_li"] }, null, null, {
                ...
                _div = sue.apps.domCreate(
                    ...
                    null,
                    null,
                    { setName: ["action", "id"], setValue: ["notepad-switch", i] }
                );
            _li.appendChild(_div);
            _div.appendChild(
                sue.apps.domCreate(
                    "span",
                    { setName: ["className"], setValue: ["su_notepad_lispan"] },
                    null,
                    null,
                    { setName: ["action", "id"], setValue: ["notepad-switch", i] },
                    data[i].title
            ...
            _div.appendChild(
                sue.apps.domCreate(
                    ...
                    null,
                    { setName: ["id", "action"], setValue: [i, "notepad-edit"] }
                )
            );
            _div.appendChild(
                sue.apps.domCreate(
                    ...
                    "display:inline-block;",
                    null,
                    { setName: ["id", "action"], setValue: [i, "notepad-del"] }
                )
            );
            _ul.appendChild(_li);
        }
    ...
    contentSave: e => {
        const ele = e.target || e;
        console.log(ele);
        sue.apps.notepad.cons.message.data.item[Number.parseInt(ele.dataset.id)].content = sue.apps
            .getAPPboxEle(e)
    ...
    showLock: e => {
        const ele = e.target || e;
        const dom = sue.apps.getAPPboxEle(ele).querySelector(".su_notepad_lock");
        dom.textContent = "";

        const _time = new Date().getTime();
        const lockCheck = sue.apps.domCreate("input", {
                setName: ["className", "type", "id"],
                setValue: ["su_notepad_locksw", "checkbox", "su_notepad_locksw" + _time],
            }),
            lockLabel = sue.apps.domCreate(
        ...
        const spanenablePw = sue.apps.domCreate(
                "span",
                { setName: ["className"], setValue: ["su_notepad_lockspan"] },
                null,
                null,
                null,
                "input password:"
            ),
            enablePw = sue.apps.domCreate("input", {
                setName: ["className", "type"],
                setValue: ["su_notepad_lockenable_pw", "password"],
            }),
            spanenablePwre = sue.apps.domCreate(
                "span",
                { setName: ["className"], setValue: ["su_notepad_lockspan"] },
                null,
                null,
            ...
            enablePwre = sue.apps.domCreate("input", {
                setName: ["className", "type"],
                setValue: ["su_notepad_lockenable_pwre", "password"],
            }),
            enableBtn = sue.apps.domCreate(
                "button",
                null,
                null,
                null,
                { setName: ["action"], setValue: ["notepad-lockenable"] },
                sue.apps.i18n("btn_done")
            ),
            spandisablePw = sue.apps.domCreate(
                "span",
                { setName: ["className"], setValue: ["su_notepad_lockspan"] },
                null,
                null,
            ...
            disablePw = sue.apps.domCreate("input", {
                setName: ["className", "type"],
                setValue: ["su_notepad_lockdisable_pw", "password"],
            }),
            disableBtn = sue.apps.domCreate(
                "button",
                null,
                null,
                null,
                { setName: ["action"], setValue: ["notepad-lockdisable"] },
                sue.apps.i18n("btn_done")
            );
...
chrome.runtime.sendMessage({ type: "apps_getvalue", apptype: "notepad" }, response => {
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    ...
    switch (message.type) {
        case "appsListener_get":
            ...
            sue.apps.notepad.listInit();
            sue.apps.notepad.itemSwitch(
                sue.apps.notepad.dom.querySelectorAll(".su_notepad_list li>div")[
                    sue.apps.notepad.config.n_notepad_last ? sue.apps.notepad.cons.message.data.last : 0
                ],
                false
            );
            break;
