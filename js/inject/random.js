...
sue.apps.random = {
    ...
    initUI: () => {
        ...
        const _node = {
            printText: sue.apps.domCreate("input", {
                setName: ["id"],
                setValue: ["su_random_print"],
            }),
            brPrint: sue.apps.domCreate("br"),
            boundSpan: sue.apps.domCreate(
                "span",
                { setName: ["className"], setValue: ["su_random_span"] },
                null,
                null,
                null,
                sue.apps.i18n("app_random_bound")
            ),
            minText: sue.apps.domCreate("input", {
                setName: ["id", "className"],
                setValue: ["min", "su_random_text"],
            }),
            dashSpan: sue.apps.domCreate("span", null, null, null, null, "~"),
            maxText: sue.apps.domCreate("input", {
                setName: ["id", "className"],
                setValue: ["max", "su_random_text"],
            }),
            brBound: sue.apps.domCreate("br"),
            numSpan: sue.apps.domCreate(
                "span",
                { setName: ["className"], setValue: ["su_random_span"] },
                null,
                null,
                null,
                sue.apps.i18n("app_random_num")
            ),
            numText: sue.apps.domCreate("input", {
                setName: ["id", "className"],
                setValue: ["num", "su_random_text"],
            }),
            brNum: sue.apps.domCreate("br"),
            norepeatCheck: sue.apps.domCreate("input", {
                setName: ["id", "className"],
                setValue: ["su_random_norepeat", "su_random_chkbox"],
            }),
            ...
            fillCheck: sue.apps.domCreate("input", {
                setName: ["id", "className"],
                setValue: ["su_random_add", "su_random_chkbox"],
            }),
            fillLabel: sue.apps.domCreate("label", null, null, null, null, sue.apps.i18n("app_random_fill")),
            brFill: sue.apps.domCreate("br"),
            btn: sue.apps.domCreate(
                "button",
                { setName: ["className"], setValue: ["su_random_btn"] },
                null,
                null,
                null,
    ...
    randnum: e => {
        ...
        var randLength = Number.parseInt(domRandom.querySelector("#num").value),
            min = Number.parseInt(domRandom.querySelector("#min").value),
            max = Number.parseInt(domRandom.querySelector("#max").value),
        ...
        for (var i = 0; i < randLength; i++) {
            var num = Number.parseInt(Math.random() * (max - min + 1) + min - (min < 0 ? 1 : 0));
            for (var ii = 0; norepeat && ii < nums.length; ii++) {
                if (num == nums[ii]) {
                    flag = true;
                    return sue.apps.random.randnum(e);
                }
            }
            nums.push(num);
        }
