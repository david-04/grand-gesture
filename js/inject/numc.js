...
sue.apps[appName] = {
    ...
    initUI: () => {
        ...
        const _spanFrom = sue.apps.domCreate(
                "span",
                { setName: ["className"], setValue: ["su_numc_span"] },
                null,
                null,
                null,
                sue.apps.i18n("app_numc_from")
            ),
            _selectFrom = sue.apps.domCreate("select", {
                setName: ["className"],
                setValue: ["su_numc_intype"],
            }),
            _brFrom = sue.apps.domCreate("br"),
            _textInput = sue.apps.domCreate("input", {
                setName: ["className"],
                setValue: ["su_numc_input"],
            }),
            _spanTo = sue.apps.domCreate(
                "span",
                { setName: ["className"], setValue: ["su_numc_span"] },
                null,
                null,
                null,
                sue.apps.i18n("app_numc_to")
            ),
            _selectTo = sue.apps.domCreate("select", {
                setName: ["className"],
                setValue: ["su_numc_outtype"],
            }),
            _brTo = sue.apps.domCreate("br"),
            _textOutput = sue.apps.domCreate("input", {
                setName: ["className"],
                setValue: ["su_numc_output"],
            }),
            _btn = sue.apps.domCreate(
                "button",
                { setName: ["className"], setValue: ["su_numc_btn"] },
                null,
                null,
                null,
        ...
        for (var i = 2; i < 33; i++) {
            var _optionFrom = sue.apps.domCreate(
                    "option",
                    { setName: ["value"], setValue: [i.toString()] },
                    null,
                    null,
                    null,
                    i.toString()
                ),
                _optionTo = sue.apps.domCreate(
                    "option",
                    { setName: ["value"], setValue: [i.toString()] },
                    null,
                    null,
    ...
    convert: e => {
        ...
        var to10 = (num, type) => Number.parseInt(num, type);
        if (from == "10") {
