...
sue.apps.tablist = {
    ...
    initUI: function () {
        ...
        for (var i = 0; i < this.list.length; i++) {
            var inner =
                "<img src='" +
                this.list[i].favIconUrl +
                "'>" +
                this.list[i].title +
                "<span class='su_tablist_button_liclose'>x</span>";
            var _li = sue.apps.domCreate(
                    "li",
                    { setName: ["className"], setValue: ["su_tablist_li"] },
                    null,
                    null,
                ...
                _span = sue.apps.domCreate(
                    "span",
                    { setName: ["className"], setValue: ["su_tablist_button_liclose"] },
                    null,
                    null,
                    null,
                    "x"
    ...
    handleEvent: e => {
        ...
        switch (e.type) {
            case "click":
                ...
                if (
                    e.target.classList.contains("su_tablist_li") ||
                    (e.target.parentNode.classList.contains("su_tablist_li") &&
                        !e.target.classList.contains("su_tablist_button_liclose"))
                ) {
...
chrome.runtime.sendMessage({ type: "apps_getvalue", apptype: "tablist" }, response => {
