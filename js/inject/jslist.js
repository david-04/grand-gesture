...
sue.apps.jslist = {
    ...
    initUI: function () {
        ...
        const _jsAll = sue.apps.jslist.js,
            ...
            for (var ii = 0; ii < _jsAll.length; ii++) {
                if (_enabled[i] == ii.toString()) {
                    var _li = sue.apps.domCreate(
                        "li",
                        { setName: ["className"], setValue: ["su_jslist_li"] },
                        null,
                        null,
    ...
    editMode: e => {
        if (e.target.classList.contains("su_jslist_editmode")) {
            ...
            sue.apps.getAPPboxEle(e).querySelector(".su_title").textContent =
                sue.apps.i18n("jslist") + " (" + sue.apps.i18n("editmode") + ")";
            sue.apps.jslist.editModeInit(e);
        }
    },
    editModeInit: e => {
        const _ulenabled = sue.apps.getAPPboxEle(e).querySelector(".su_jslist_ulenabled");
        ...
        for (var i = 0; i < _jsAll.length; i++) {
            if (!_enabled.contains(i)) {
                var _li = sue.apps.domCreate("li", { setName: ["className"], setValue: ["su_jslist_li"] }, null, null, {
                    setName: ["id"],
                    setValue: [i],
                });
                var _div = sue.apps.domCreate(
                    "div",
                    { setName: ["className"], setValue: ["su_jslist_li su_jslist_item"] },
                    null,
                    null,
        ...
        sue.apps.jslist.cons.sortDomEnabled = Sortable.create(_ulenabled, {
            ...
            onEnd: e => {
                console.log(e);
                const _lis = e.target.querySelectorAll("li.su_jslist_li");
                const _arr = [];
...
chrome.runtime.sendMessage({ type: "apps_getvalue", apptype: "jslist" }, response => {
